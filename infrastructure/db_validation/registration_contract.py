...
def validate_registration_insert_contract(engine, orm_model: DeclarativeMeta):
    '''
    Docstring for validate_registration_insert_contract
    ...
    for column in table.columns:
        ...
        if column.name not in db_columns:
            raise RuntimeError(
                f"Column '{column.name}' does not exist in table 'registro_comercio'."
                )
        
        # If necessary column is nullable in DB, raise error
        if db_columns[column.name]["nullable"]:
            raise RuntimeError(
                f"Column '{column.name}' in table 'registro_comercio' must be NOT NULL."
                )